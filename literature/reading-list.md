...
目前处于[执行路线](../project/roadmap.md)的 A–B 准备阶段，尚无数值结果。推荐先读 **Chen → Langheld → Mendonça 与 Comment 对读 → Leibig 的模型映射**。有限温度论文先了解模型差异，正式进入该路线前再精读方法；不必等全部 20 篇通读后才建立最小模型。
...
这篇帮助我们建立光场部分的理论基线。第一遍追踪 Rabi 哈密顿量、A²、反压缩与图 1 中的相区；第二遍对照补充材料手推变换。其 NMR 实验模拟有效模型基态，不能直接证明任意光学腔都具备独立静态 `ξ`。其临界极限是单自旋的频率比趋于无穷，与多自旋 `N→∞` 要分别标注。

读后自查：为什么两个二次项可以合并、物理控制路径仍可不同？哪些小 `g` 参数点属于不稳定区？变换后的光子数为什么不能直接当原光场占据？对应笔记进入计划中的 `theory/model.md` 和 `theory/derivations.md`；本文记录原文位置与尚未弄懂的问题。初期可将 NMR 脉冲实现细节放到后读。

### Langheld：决定我们应该复现什么，以及哪些不能当作创新

这篇是当前 Ising 相图的首要数值参照。建议阅读顺序是**主文模型 → 补充材料 A² 节 → 主文相图 → 跃迁阶数/尺寸分析**。特别注意图 S9 的物理约束示例；“在 Ising 模型中加入 A²”已经有明确先例。完整 wormhole 更新算法不是第一轮小系统 ED 的前置要求。

读后自查：项目中 `Δ` 与文中 `ε` 相差多少？图中哪条路径与本项目相同？共存相同时具备哪两种序？相界依据是什么？首轮产物是一个带参数、符号换算和图号的基准定义，而非仅凭图形外观说复现成功。

### Mendonça 与 Comment：学习如何处理相图争议

Mendonça 是用户原始选题的重要来源，涉及 Ising/XXZ 和混合变分方法；Comment 对其部分 Ising 相图提出共存区及跃迁阶数方面的异议。先把争议限定到相同哈密顿量、参数范围和极限，再比较证据；不能把对 Ising 的质疑自动扩展成否定整篇论文，也不能因发表期刊而免于核查。

读后自查：双方实际比较的参数是否一致？窄共存区可能怎样被网格或近似漏掉？哪些结论需要能量分支/大系统数据才能判定？本项目先读 Ising 部分，XXZ 指数和完整非高斯变分方法等选定 XXZ 路线后再深读。

### Leibig：为热力学计算选方法

这篇预印本把热力学相图问题转为自洽的量子自旋模型，并结合 NLCE+DMRG 求解。现在重点读 §2.2 的映射和能量极值条件；进入 D 阶段后再读自洽分支、反铁磁处理和结果比较。

读后自查：求得一个自洽根是否就找到基态？保留了哪些自旋关联？哪些有限系统的光子涨落不能由热力学静态解直接替代？产物是 `f(x)` 的求解流程和验收点；方法难度较高时先采用小系统校验，不以完整实现该算法作为开始研究的条件。

### Gammelmark–Mølmer：在选择有限温度路线前划清问题

其量子 Ising–腔模型已有有限温度自由能及不同阶数的边界。先核对静态场、交换方向和腔耦合方向；本项目拟定模型不能未经检查就套用它的可解链公式。之后再读 §3–4，看配分函数如何形成自由能以及如何选择稳定分支。

读后自查：本题与该文的具体差别在哪个算符/约束？有限温度想检验什么额外预测？第一批小系统热 ED 应核对哪个极限？这些问题未答清前，有限温度继续保留为候选。

### 随问题补读的文献

| 触发问题 | 补读文献 | 现在的读法 |
| --- | --- | --- |
| 解释禁戒和平台约束 | Nataf2010NoGo；若选电路再结合 Zhang2014CircuitIsing | A2 阶段先核前提与有效模型来源，不把一个平台的求和规则套给所有平台 |
| 判断“临界增强/压缩协同”是否已知 | Sur2026Critical | 进入 E1 前核模型、主结论与本题候选量的关系 |
| 要拟合三临界点或临界指数 | Koziol2026Criticality，以及 Langheld 补充材料的标度讨论 | 有可收敛的数据和具体拟合问题后再深入，先核适用的尺寸标度形式 |
| 连接真实热平衡材料或能谱 | Bamba2022Magnonic、Kim2025Magnonic、RomanRoche2025Bound | 平台与观测量确定时读；磁振子角色与普通腔光子要区分 |
| 选择有限温度或驱动路线 | Peng2019Unified、Otake2026FiniteT；Zhu2020SqueezedLight、Zhu2024Nonreciprocal | 对应路线启动时精读。用户上传的非互易论文中 `J` 是模间耦合，不能当本题自旋交换 |

阅读进展统一写在本文对应条目内，使用“已定位关键段落／已复算公式／已数值复现”描述证据；这三种深度不同。以上安排是建议给用户的阅读顺序，不意味着用户已经完成阅读。当前所有 20 篇的内容概述和核读深度继续见下文。
