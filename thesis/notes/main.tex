...
目前拟研究静态、闭合有效哈密顿量的平衡态；以零温基态为基线，有限温度热平衡可以成为主线。
...
先在 $T=0$ 求反压缩启动阈值与稳定窗口，再以配分函数 $Z=\operatorname{Tr}e^{-\beta H}$ 与自由能
$F=-\beta^{-1}\ln Z$ 研究有限温度相界、临界温度和跃迁阶数，检查随 $g$ 变化是否反向穿越一级边界或进入共存区。
...
\begin{enumerate}
  ...
  \item 对有限温度计算明确热力学极限及数值方法，并与腔耦合 Ising 的现有热相变结果对照。
